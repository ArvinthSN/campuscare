...
const Navigation = ({ activeSection, setActiveSection }: NavigationProps) => {
  ...
  return (
    <nav className="glass fixed top-0 left-0 right-0 z-50 backdrop-blur-md px-4 sm:px-6 py-3 sm:py-4">
      {/* top row: left (logo), center (nav), right (user) */}
      <div className="max-w-full mx-auto flex items-center gap-3">
        ...
        {/* CENTER: Nav items (flexible; must have min-w-0 so overflow works) */}
        <div className="flex-1 min-w-0 mx-3">
          {/* Desktop nav: single-line, scrollable if needed */}
            ...
            {navItems.map((item) => {
              ...
              return (
                <Button
                  ...
                  // whitespace-nowrap + inline-flex keeps the button on one line
          ...
          {/* Optional: small-screen variant for tablet widths: icons-only */}
          {/* <div className="hidden lg:hidden md:flex items-center gap-2 justify-center"> ...icons-only buttons ...</div> */}
        </div>

        {/* RIGHT: user info & theme (fixed) */}
        <div className="flex items-center gap-3 flex-shrink-0">
